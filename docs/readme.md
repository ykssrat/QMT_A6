...
**收益率与已实现盈亏口径说明**：
- `total_return`（总收益率）按组合净值变化计算，包含了**已卖出兑现的盈亏**，也包含了**当前仍持仓、尚未卖出的浮动盈亏**
- `realized_pnl`（已实现盈亏）只统计卖出成交时真正落袋的盈亏，不包含任何仍在持仓中的浮盈或浮亏
- 因此会出现一种看起来“很难绷”但统计上完全正常的情况：`total_return` 很高，但 `realized_pnl` 仍然为负。这通常表示当前组合账面浮盈很多，但历史上真正卖出的交易总体还在亏钱
- 当前系统在“参数调优 / 独立荐股评分”上优先使用 `realized_pnl`，因为它更接近“真正兑现到现金里的结果”；而 `total_return` 继续保留，是为了观察组合整体净值表现
- 可以把两者理解为：`total_return` 看的是“现在账面一共涨了多少”，`realized_pnl` 看的是“真正卖掉后一共赚了多少钱”
...
- 回测阅读提示：若看到“收益率为正，但已实现盈亏为负”，不要直接当成脚本错误。通常这表示当前仍持有的仓位账面浮盈较多，但已经卖出的历史交易合计仍未转正；两者统计口径不同，前者含浮盈，后者只算已卖出成交
